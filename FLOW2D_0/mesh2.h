...
public:
	...
	MESHVertex const & get_vertex(unsigned const i)		const;
	MESHEdge const & get_edge(unsigned const i)		const;
	MESHTriangle const & get_triangle(unsigned const i)	const;
...
void MESH::allocate_new_primitives(Triangulation<GK> & triangulation) {
	...
	for (size_t i = 0; i < NumberOf_Triangles; i++) {
		...
		MESHVertex const a = vertices[A->index];
		MESHVertex const b = vertices[B->index];
		MESHVertex const c = vertices[C->index];

		MESHTriangle new_triangle(a, b, c);
...
MESHVertex const & MESH::get_vertex(unsigned const i) const {

	return vertices[i];

}
MESHEdge const & MESH::get_edge(unsigned const i) const {

	return edges[i];

}
MESHTriangle const & MESH::get_triangle(unsigned const i) const {
...
void MESH::export_triangles(std::ofstream & stream) const {
	...
	for (size_t i = 0; i < triangles.size(); i++) {
		...
		MESHTriangle const t = (triangles[i]);
